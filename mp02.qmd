...
# Data Acquisition
Income, Rent, Population, and Household Dataset
```{r}
#| message: false
...
format_titles <- function(df){
  ...
  df}
```
### EC: Relationship Diagram

![](mp02_erd.png)
note: Wages's INDUSTRY can be joined with INDUSTRY_CODES level1_code, level2_code, level3_code, and level4_code, but the program I used doesn't allow me to create multiple connections. 
...
#| warning: false
  ...
  POPULATION |>
    ...
    ggplot(aes(x = year, y = avg_housesize, group = GEOID, color = as.factor(GEOID))) +
    geom_line(alpha = 0.5, size = 0.7) +
    gghighlight(GEOID %in% c(35620, 31080),
                label_key = GEOID,
                use_direct_label = TRUE,
                unhighlighted_params = list(alpha = 0.3, color = "grey80")) +
  scale_color_manual(values = c("35620" = "tomato", "31080" = "steelblue")) +
    xlab("Year") + 
    ylab("Average Household Size") + 
    labs(title = "Average Household Size Over Time for Each GEOID",
         caption = '35620 = NYC, 31080 = LA') +
    theme_bw() +
    theme(
    legend.position = "none",
    plot.title = element_text(hjust = 0.5, face = "bold"))
...
```
While for majority of the years in the study period indicate that Burlington, Vermont has a higher rent burden compared to the national average, the z score is relatively low. This tells us that in comparison to the national average, Burlington is relatively an affordable city to reside in. 
...
T6_dt <- highest_hgrate_cities |>
  filter(max(HG_rate_zscore) > 3)|>
...
This is the same graph as the one above, but the purple indicates the GEOIDs that has population growth over the study period
```{r}
#| warning: false
...
### 3. Cities with the Highest Rent Burden Score

This visualization showcases the US CBSA's with the highest rent burden score, and most of them are Puerto Rico areas. Let's filter out PR areas, and examine the top CBSA's from the 50 states with the highest rent burden score. 
```{r}
rb_standardized |>
...
T6_rb_pr <- rb_standardized |>
  select(GEOID, NAME.x, rb_zscore) |>
  arrange(desc(rb_zscore)) |>
...
### 3.2 Cities with the Highest Rent Burden Score (excluding Puerto Rico)

Interesting! This visualization shows that cities in Florida rank among the most expensive places to live.
```{r}
...
T6_nopr <- states_rb_standardized|>
  select(GEOID, NAME.x, rb_zscore) |>
  arrange(desc(rb_zscore)) |>
  distinct(GEOID, .keep_all = TRUE) |>
  separate(NAME.x, into = c('City', 'State'), sep = ',\\s')|> 
