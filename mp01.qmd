...
### Top 10 Longest-Running Programming by Category

```{r}
# bar graph of the top 10 shows with the longest run in Global top 10
longest_tv_english <- GLOBAL_TOP_10 |>
  filter(category == "TV (English)") |>
  group_by(show_title) |>
  summarise(total_weeks_in_top10 = max(cumulative_weeks_in_top_10)) |>
  slice_max(total_weeks_in_top10, n = 10)

longest_tv_non <- GLOBAL_TOP_10 |>
  filter(category == "TV (Non-English)") |>
  group_by(show_title) |>
  summarise(total_weeks_in_top10 = max(cumulative_weeks_in_top_10)) |>
  slice_max(total_weeks_in_top10, n = 10)

longest_film_english <- GLOBAL_TOP_10 |>
  filter(category == "Films (English)") |>
  group_by(show_title) |>
  summarise(total_weeks_in_top10 = max(cumulative_weeks_in_top_10)) |>
  slice_max(total_weeks_in_top10, n = 10)

longest_film_non <- GLOBAL_TOP_10 |>
  filter(category == "Films (Non-English)") |>
  group_by(show_title) |>
  summarise(total_weeks_in_top10 = max(cumulative_weeks_in_top_10)) |>
  slice_max(total_weeks_in_top10, n = 10)

# Combine datasets and label categories
all_top <- bind_rows(
  longest_tv_english |> mutate(category = "TV (English)"),
  longest_tv_non |> mutate(category = "TV (Non-English)"),
  longest_film_english |> mutate(category = "Films (English)"),
  longest_film_non |> mutate(category = "Films (Non-English)"))
...
for (cat in categories) {
  df <- all_top |> filter(category == cat)|>
    mutate(show_title = factor(show_title, levels = show_title)) 
  
  fig <- fig %>% add_trace(
    data = df,
    x = ~total_weeks_in_top10,
    y = ~show_title,
    type = "bar",
    orientation = "h",
    name = cat,
    visible = ifelse(cat == categories[1], TRUE, FALSE)
  )
}

# Build buttons that update trace visibility + Y axis labels
buttons <- lapply(seq_along(categories), function(i) {
  ...
  # Y-axis labels ONLY for this category
  y_vals <- all_top %>%
    filter(category == this_cat) %>%
    pull(show_title)
...
fig <- fig %>%
  layout(
    title = "Top 10 Longest-Running Programming by Category",
    xaxis = list(title = "Total Weeks in Top 10"),
    yaxis = list(title = "Programming"),
    margin = list(l = 220), 
    updatemenus = list(
      list(
        active = 0,
...
### Number of Weeks KDH spent in the Global Top 10
```{r}
#| code-fold: true
GLOBAL_TOP_10|>
  filter(show_title == 'KPop Demon Hunters')|>
