...
baruch_point <- new_st_point(40.74028, -73.98333)
...
#Only calling for the council districts with the highest count of london trees, with Japanese Zelkova serrata tree counts
japanese_london <- tree_districts |>
...
### District 19- North Eastern Queens
...
#plotting London plane from district 46,50,19,23
featured_districts <- tree_districts |>
  mutate(top_london = case_when(
    CounDist %in% c(46,50,19,23) ~ as.character(CounDist),
    TRUE ~ "other"
  ),
  top_london = factor(top_london, levels = names(london_colors)))

ggplot(featured_districts) +
  geom_sf(aes(color = top_london), size = 0.6) +
  scale_color_manual(values = london_colors, name = "District") +
  guides(
    color = guide_legend(
      override.aes = list(size = 3))) +
  theme_minimal() +
  labs(title = "Districts with Most London plane Trees")
```
